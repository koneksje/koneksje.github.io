...
function formthrottle_check()
{
	...
	try
	{
		...
		{
			$db = new PDO('sqlite:koneksje-throttle-db.sqlite3');
			if ( file_exists('koneksje-throttle-db') )
			{
				unlink('koneksje-throttle-db');
			}
		}
		else if (function_exists("sqlite_open")) 
		{
			$db = new PDO('sqlite2:koneksje-throttle-db');
			if ( file_exists('koneksje-throttle-db.sqlite3') )
			{
				unlink('koneksje-throttle-db.sqlite3');
			}
		} else {
			return '4';
		}
